...
test.describe("Resume Yatra → onboarding app redirect", () => {
  test.beforeEach(async ({ page }) => {
    await installOnboardingRedirectMocks(page, {
      resumeYatra: { ryOnboarded: false },
    });
  });
  ...
  test("redirects authenticated user with incomplete product onboarding to onboarding app", async ({
    ...
    expect(u.searchParams.get("productId")).toBe("resume-yatra");
    expect(u.searchParams.get("from")).toBe("resumeyatra");
