...
test.describe("DSA Yatra → onboarding app redirect", () => {
  test.beforeEach(async ({ page }) => {
    await installOnboardingRedirectMocks(page, {
      dsaYatra: { dyOnboarded: false },
    });
  });
  ...
  test("redirects authenticated user with incomplete product onboarding to onboarding app", async ({
    ...
    expect(u.searchParams.get("productId")).toBe("dsayatra");
    expect(u.searchParams.get("from")).toBe("dsayatra");
