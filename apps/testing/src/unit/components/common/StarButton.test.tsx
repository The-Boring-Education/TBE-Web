...
vi.mock('react-icons/fa', () => ({
    FaStar: ({ className }: any) => <svg className={className} data-testid="star-icon" />,
}));
...
describe('StarButton Component', () => {
    ...
    describe('Interaction', () => {
        ...
        it('should be disabled when isLoading is true', () => {
            render(<StarButton isStarred={false} onToggle={vi.fn()} isLoading={true} />);
            const button = screen.getByRole('button');
            expect(button).toBeDisabled();
        });

        it('should not be disabled when isLoading is false', () => {
            render(<StarButton isStarred={false} onToggle={vi.fn()} isLoading={false} />);
            const button = screen.getByRole('button');
    ...
    describe('Props', () => {
        it('should apply custom className', () => {
            const { container } = render(
                <StarButton isStarred={false} onToggle={vi.fn()} className="custom-class" />
            );
            const button = container.querySelector('button');
