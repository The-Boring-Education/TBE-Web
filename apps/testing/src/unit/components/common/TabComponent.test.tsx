...
vi.mock('@headlessui/react', () => {
    ...
    const TabComponent = ({ children, className, selected = false }: any) => {
        const classNameValue = typeof className === 'function' ? className({ selected }) : className;
        return (
    ...
    TabComponent.Group = ({ children, className }: any) => (
        <div className={className} data-testid="tab-group">{children}</div>
    );
    TabComponent.List = ({ children, className }: any) => (
        <div className={className} data-testid="tab-list" role="tablist">{children}</div>
    );
    TabComponent.Panels = ({ children }: any) => (
        <div data-testid="tab-panels">{children}</div>
    );
    TabComponent.Panel = ({ children, className }: any) => (
        <div className={className} data-testid="tab-panel">{children}</div>
    );
...
describe('TabComponent', () => {
    describe('Rendering', () => {
        it('should render tab component with labels and panels', () => {
            ...
            const tabPanels = [<div key="1">Panel 1</div>, <div key="2">Panel 2</div>];

            const { container } = render(
                <TabComponent tabLabels={tabLabels} tabPanels={tabPanels} />
            );
            
            expect(container.querySelector('[data-testid="tab-group"]')).not.toBeNull();
            expect(container.querySelector('[data-testid="tab-list"]')).not.toBeNull();
            expect(container.querySelector('[data-testid="tab-panels"]')).not.toBeNull();
        });
    ...
    describe('Vertical Layout', () => {
        it('should apply vertical layout classes when vertical is true', () => {
            ...
            const { container } = render(
                <TabComponent tabLabels={tabLabels} tabPanels={tabPanels} vertical={true} />
            );
        ...
        it('should apply horizontal layout classes when vertical is false', () => {
            ...
            const { container } = render(
                <TabComponent tabLabels={tabLabels} tabPanels={tabPanels} vertical={false} />
            );
