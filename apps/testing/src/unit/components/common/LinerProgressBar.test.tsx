...
describe("LinerProgressBar Component", () => {
  it("shows correct text for partial completion", () => {
    ...
    expect(
      screen.getByText("5 / 10 Chapters Completed (50%)"),
    ).toBeInTheDocument();
  });
  ...
  it("shows 0% when totalChapters is 0 (edge case)", () => {
    ...
    expect(
      screen.getByText("0 / 0 Chapters Completed (0%)"),
    ).toBeInTheDocument();
  });
  ...
  it("shows 100% when all chapters complete", () => {
    ...
    expect(
      screen.getByText("8 / 8 Chapters Completed (100%)"),
    ).toBeInTheDocument();
  });
  ...
  it("handles 0 completed chapters", () => {
    ...
    expect(
      screen.getByText("0 / 5 Chapters Completed (0%)"),
    ).toBeInTheDocument();
  });
  ...
  it("uses Math.floor (e.g., 1/3 = 33%, not 33.33%)", () => {
    ...
    expect(
      screen.getByText("1 / 3 Chapters Completed (33%)"),
    ).toBeInTheDocument();
  });
});
