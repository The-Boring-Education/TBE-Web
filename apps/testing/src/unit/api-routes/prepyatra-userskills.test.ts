...
describe('PrepYatra UserSkills API Route', () => {
    ...
    describe('POST /api/v1/prepyatra/userskills', () => {
        it('should add skills for a valid ObjectId userId', async () => {
            ...
            mockUpdateUserSkillsInDB.mockResolvedValue({ data: { _id: validObjectId }, error: null });
            ...
            expect(mockUpdateUserSkillsInDB).toHaveBeenCalledWith(validObjectId, ['React', 'TypeScript']);
        ...
        it('should add skills for an email userId by looking up the user', async () => {
            ...
            mockUpdateUserSkillsInDB.mockResolvedValue({ data: { _id: mongoId }, error: null });
            ...
            expect(mockUpdateUserSkillsInDB).toHaveBeenCalledWith(mongoId, ['Node.js']);
            expect(res._getStatusCode()).toBe(200);
            const data = JSON.parse(res._getData());
            expect(data.status).toBe(true);
        });
    ...
    describe('DELETE /api/v1/prepyatra/userskills', () => {
        it('should remove a skill successfully', async () => {
            const validObjectId = '507f1f77bcf86cd799439011';
            mockUserFindByIdAndUpdate.mockResolvedValue({ _id: validObjectId, userSkills: ['React'] });
        ...
        it('should handle removing a skill by email lookup', async () => {
            ...
            const mongoId = '507f1f77bcf86cd799439011';
            mockUserFindOne.mockResolvedValue({ _id: mongoId });
            mockUserFindByIdAndUpdate.mockResolvedValue({ _id: mongoId, userSkills: [] });
