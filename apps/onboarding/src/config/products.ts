...
export const PRODUCT_CONFIGS: Record<string, OnboardingProductConfig> = {
  webapp: {
    ...
    api: {
      endpoint: (userId: string) =>
        `${config.API_BASE_URL}/user/onboarding?userId=${userId}`,
      method: "POST",
  ...
  prepyatra: {
    ...
    api: {
      endpoint: () => `${config.API_BASE_URL}/prepyatra/onboarding`,
      method: "POST",
  ...
  dsayatra: {
    ...
    fields: [
      ...
      createField("target", "Choose your target", "select", 6, {
        ...
        prefill: {
          fromUser: (user: User) => user.dsaYatra?.target || "",
        },
      }),
      createField("targetTopics", "Target DSA Topics", "multiselect", 7, {
        ...
        prefill: {
          fromUser: (user: User) => user.dsaYatra?.targetTopics || [],
        },
      }),
    ],
    api: {
