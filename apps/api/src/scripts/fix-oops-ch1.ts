...
<img src="https://lh3.googleusercontent.com/d/1hQfuOCPF8rwusYqXns6Ppvyje_etMZwS" alt="Class and Object Memory Layout" style="max-width:100%;height:auto;border-radius:12px;margin:20px auto;display:block;border:1px solid #1f2937;" loading="lazy" />
...
async function fix() {
  ...
  if (after) {
    const ch = after.chapters.find((c: any) => c.title === "Classes & Objects");
    if (ch) {
      const hasImage =
        ch.content?.markdownContent?.includes("drive.google.com");
      const hasCorrectPosition = ch.content?.markdownContent?.includes(
        "Object**: The actual physical house constructed from the blueprint. It is a concrete instance that exists in memory.\n\n<img",
      );
      console.log("✅ Google Drive image URL present:", hasImage);
      console.log("✅ Image is before Core Properties:", hasCorrectPosition);
    }
  }
