      ...
      if (currentStreak === milestone && previousStreak < milestone) {
        try {
          ...
          await handleGamificationPoints(
            true,
            userId,
            `PREPLOG_STREAK_${milestone}` as Parameters<
              typeof handleGamificationPoints
            >[2],
          );
...
const getAllUsersWithLogsFromDB =
  async (): Promise<DatabaseQueryResponseType> => {
    try {
      ...
      // Map userId to logs
      const logsMap: Record<string, (typeof logs)[number][]> = {};
...
const getAllMenteesFromDB = async (): Promise<DatabaseQueryResponseType> => {
  try {
    ...
    type MenteeDoc = (typeof mentees)[number] & {
      user: {
        _id: unknown;
        prepYatra?: { prepLog?: Record<string, unknown> };
        [key: string]: unknown;
      };
      selectedAt?: Date;
      note?: string;
    ...
    const userIds = (mentees as MenteeDoc[]).map((m) => String(m.user._id));
    ...
    const menteesWithDetails = (mentees as MenteeDoc[]).map((m) => {
      const user = m.user;
      return {
        ...user,
        logs: logsMap[user._id.toString()] || [],
...
const getChallengeProgressFromDB = async (
  ...
  try {
    ...
    for (let i = 0; i < sortedLogs.length; i++) {
      ...
      if (
        i === 0 ||
        (currentLog && previousLog && currentLog.day === previousLog.day + 1)
      ) {
        streak++;
      } else {
        streak = 1;
