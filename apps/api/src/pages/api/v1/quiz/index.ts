...
async function handleCreateQuiz(req: NextApiRequest, res: NextApiResponse) {
    ...
    if (quizId) {
        ...
        const { data, error } = await appendQuestionsToQuizInDB(
            quizId,
            questions
        )
    ...
    if (!categoryName || !categoryDescription || !categoryIcon || !questions) {
        return res.status(400).json({
            error: "Missing required fields: categoryName, categoryDescription, categoryIcon, questions"
        })
    }
    ...
    const errors: string[] = []
    const normalizedQuestions = (questions as PartialQuestion[]).map(
        (q, idx) => {
            ...
            const difficultyValid = ["easy", "medium", "hard"].includes(
                difficulty
            )
            ...
            if (missing.length) {
                errors.push(
                    `Question[${idx}] missing/invalid: ${missing.join(", ")}`
                )
            } else if (
