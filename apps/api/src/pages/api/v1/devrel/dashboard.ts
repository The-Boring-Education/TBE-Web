...
const handleAdvocateDashboard = async (
  ...
  try {
    ...
    const applicationCounts = {
      total: stats.total,
      pending:
        stats.statusCounts.find((s: StatusCount) => s.status === "applied")
          ?.count || 0,
      approved:
        stats.statusCounts.find((s: StatusCount) => s.status === "approved")
          ?.count || 0,
      rejected:
        stats.statusCounts.find((s: StatusCount) => s.status === "rejected")
          ?.count || 0,
    };
    ...
    const dashboardData = {
      ...
      leads: {
        ...
        active: accessibleLeads.filter((lead) => lead.status === "onboarded")
          .length,
...
const handleLeadDashboard = async (
  ...
  try {
    ...
    const allTasks = await DevRelTask.findForLead(lead._id.toString()).populate(
      "createdBy",
      "name email",
    );
    ...
    const onboardingProgress = {
      ...
      percentage:
        totalOnboardingTasks > 0
          ...
          : 0,
      nextTask:
        tasks.pending.find((task) => task.type === "onboarding") || null,
    };
