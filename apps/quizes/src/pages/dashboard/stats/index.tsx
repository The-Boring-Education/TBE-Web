        ...
        >
          ...
          <div className="flex-1">
            ...
            <div className="flex items-center space-x-6 text-sm">
              <span>
                Best: <strong>{category.bestScore}%</strong>
              </span>
              <span>
                Avg: <strong>{category.averageScore}%</strong>
              </span>
              <span>
                Time: <strong>{category.totalTimeSpent} seconds</strong>
              </span>
            </div>
          </div>
          <div
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          ...
          <MetricCard
            title="Average Score"
            value={
              metricsLoading ? <MetricLoader /> : `${metrics.averageScore}%`
            }
            subtitle="Overall performance"
            icon={<BarChart3 className="w-6 h-6 text-green-500" />}
            trend={{
              value: metrics.improvementRate ?? 0,
              isPositive: (metrics.improvementRate ?? 0) > 0,
            }}
            className="animate-slide-in-left"
          ...
          <MetricCard
            title="Streak Days"
            value={
              metricsLoading ? <MetricLoader /> : (metrics.streakDays ?? 0)
            }
          ...
          <Card className="glass animate-scale-in">
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="text-center p-4 bg-muted/30 rounded-lg">
                  <p className="text-2xl font-bold text-foreground">
                    {metricsLoading ? "..." : (metrics.averageTimePerQuiz ?? 0)}{" "}
                    seconds
                  </p>
            ...
            <CardContent>
              <div className="space-y-3">
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <span className="text-sm">
                    Total points earned: {metrics.totalScore ?? 0}
                  </span>
                </div>
                <div className="flex items-center space-x-3">
                  <div className="w-2 h-2 bg-purple-500 rounded-full" />
                  <span className="text-sm">
                    Questions answered: {metrics.totalAttempts * 10}
                  </span>
                </div>
              </div>
            </CardContent>
