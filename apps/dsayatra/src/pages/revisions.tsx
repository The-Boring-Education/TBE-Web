...
export default function RevisionsUI({ seoMeta }: PageProps) {
    ...
    useEffect(() => {
        ...
        if (savedGlobal) {
            try { setGlobalCompleted(JSON.parse(savedGlobal)); } catch { }
        }

        const savedAssignments = localStorage.getItem('dsayatra_weekly_revisions');
        if (savedAssignments) {
            try { setWeeklyAssignments(JSON.parse(savedAssignments)); } catch { }
        }

        const savedProgress = localStorage.getItem('dsayatra_revision_completed');
        if (savedProgress) {
            try { setWeekProgress(JSON.parse(savedProgress)); } catch { }
        }
    }, []);
    ...
    useEffect(() => {
        ...
        for (let i = 0; i < numUnlockedWeeks; i++) {
            ...
            if (validWeekQs.length < 7 && validGlobalCompleted.length > validWeekQs.length) {
                // Determine what can still be drawn. Prefer drawing unused questions.
                let availablePool = validGlobalCompleted.filter(id => !validWeekQs.includes(id) && !usedQuestions.has(id));
                ...
                if (availablePool.length < (7 - validWeekQs.length)) {
                    availablePool = validGlobalCompleted.filter(id => !validWeekQs.includes(id));
                }
        ...
        if (modified) {
            ...
            localStorage.setItem('dsayatra_weekly_revisions', JSON.stringify(newAssignments));
        }
    ...
    const handleSelectWeek = (weekIndex: number) => {
        ...
        if (!assignments[weekIndex] || assignments[weekIndex].length < 7) {
            ...
            if (currentValid.length < 7 && validGlobalCompleted.length > currentValid.length) {
                const availablePool = validGlobalCompleted.filter(id => !currentValid.includes(id));
                const shuffled = [...availablePool].sort(() => 0.5 - Math.random());
                const needed = 7 - currentValid.length;
                assignments[weekIndex] = [...currentValid, ...shuffled.slice(0, needed)];
                setWeeklyAssignments(assignments);
                localStorage.setItem('dsayatra_weekly_revisions', JSON.stringify(assignments));
            }
        }
    ...
    const toggleRevisionQuestion = (qId: string) => {
        ...
        setWeekProgress(prev => {
            ...
            const newWeekData = isCompleted ? weekData.filter(id => id !== qId) : [...weekData, qId];
        ...
        return (
            <div className="flex bg-[#0A0A0A] font-sans h-[calc(100vh-72px)]">
                <main className="flex-1 flex items-center justify-center">
                    ...
                    <Text level="p" className="text-gray-400 ml-3">Loading Revisions...</Text>
                </main>
            </div>
        );
    }
    ...
    if (selectedWeek === null) {
        return (
            <Fragment>
                ...
                <FlexContainer direction="col" className="flex-1 min-h-screen w-full bg-[#0A0A0A] mt-0 font-sans px-4 sm:px-8 py-8" itemCenter={false} justifyCenter={false} wrap={false}>
                    <div className="max-w-[1400px] w-full mx-auto">
                        <header className="mb-10 text-center sm:text-left">
                            <h1 className="text-3xl font-bold text-white mb-2">Weekly Revisions</h1>
                            <p className="text-sm text-gray-400">Unlock a new revision week for every 10 questions you complete in the sheet!</p>
                            <p className="text-sm text-[#ff5757] font-semibold mt-1">Total Completed: {globalCompleted.length} / {totalWeeks * 10} for fully unlocked.</p>
                        </header>

                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                            {Array.from({ length: totalWeeks }).map((_, i) => {
                                const isUnlocked = i < numUnlockedWeeks;
                                const weekQuestions = (weeklyAssignments[i] || []).filter(id =>
                                    dsaQuestions.some(q => String(q.id || q.name) === String(id))
                                );
                                const weekDone = weekProgress[i] || [];
                                const isCompleted = weekQuestions.length > 0 && weekDone.length === weekQuestions.length;
                                ...
                                return (
                                    <div
                                        ...
                                        className={cn(
                                            ...
                                            isCompleted && "border-green-500/30 bg-green-500/5 hover:border-green-500/50 hover:bg-green-500/10 hover:shadow-[0_0_15px_rgba(74,222,128,0.2)]"
                                        )}
                                    >
                                        <div className="flex flex-col gap-1">
                                            <div className="flex items-center gap-2">
                                                <h3 className={cn("text-lg font-bold", isUnlocked ? "text-white" : "text-gray-500", isCompleted && "text-green-500")}>Week {i + 1}</h3>
                                                {!isUnlocked && <Lock className="w-4 h-4 text-gray-500" />}
                                                {isCompleted && <Check className="w-4 h-4 text-green-500" />}
                                            </div>
                                            <p className="text-xs text-gray-500">
                                                {isUnlocked ? (weekQuestions.length > 0 ? `${weekDone.length}/${weekQuestions.length} Completed` : "Ready to start") : `Unlocks at ${(i + 1) * 10} overall questions`}
                                            </p>
                                        </div>
                                        {isUnlocked ? <ChevronRight className="w-5 h-5 text-gray-500" /> : null}
                                    </div>
                                );
                            })}
                        </div>
    ...
    const currentQuestions = currentAssignmentIds.map(id => dsaQuestions.find(q => String(q.id || q.name) === String(id))).filter(Boolean) as DsaQuestion[];
    const completedWeekIds = weekProgress[selectedWeek] || [];
    const progress = currentQuestions.length > 0 ? (completedWeekIds.length / currentQuestions.length) * 100 : 0;
                    ...
                    <section className="grid grid-cols-1 md:grid-cols-12 gap-5 mb-10 animate-fadeUp">
                                ...
                                <div className="flip-card-front glass-panel rounded-xl p-5 relative overflow-hidden flex flex-col items-start justify-start border border-[#2a2a2a] hover:border-[#ff5757] hover:shadow-[0_0_15px_rgba(255,87,87,0.3)] transition-all duration-300">
                                    <div className="w-full flex justify-between items-start z-10">
                                        <div className="text-left">
                                            <h3 className="text-sm font-semibold text-[#ff5757]">Memory Retention</h3>
                                            <p className="text-[10px] text-gray-400">Ebbinghaus Curve Visualization</p>
                                        </div>
                                    ...
                                    <div className="w-full h-full relative -mt-4">
                                        <svg width="100%" height="100%" viewBox="0 0 500 240" preserveAspectRatio="none">
                                            {[0, 1, 2, 3].map((i) => (
                                                <g key={i}>
                                                    <line x1={i * 125 + 62.5} y1="0" x2={i * 125 + 62.5} y2="220" stroke="rgba(255,255,255,0.05)" strokeWidth="1" strokeDasharray="4 4" />
                                                    <text x={i * 125 + 62.5} y="235" fill="#8a8f9d" fontSize="10" textAnchor="middle">{i}d</text>
                                                </g>
                                            ))}
                                            <path d="M 0 50 Q 80 180, 187.5 200 T 312.5 220 T 500 225" fill="none" stroke="rgba(255,87,87,0.15)" strokeWidth="2" strokeDasharray="6 6" />
                                            <g style={{
                                            ...
                                            <defs>
                                                <linearGradient id="revision-gradient" x1="0%" y1="0%" x2="100%" y2="0%">
                                ...
                                <div className="flip-card-back text-left hover:border-[#ff5757] hover:shadow-[0_0_15px_rgba(255,87,87,0.3)] transition-all duration-300">
                                    <div>
                                        ...
                                        <p className="text-gray-300 text-sm leading-relaxed">
                                            After every three days, we begin to forget what we have newly learned.
                                            To remember and deeply retain concepts, spaced revision is required.
                                            This is why the forgetting curve drops sharply. By reviewing these questions
                                            now, you're flattening the curve and committing them to long-term memory!
                                        </p>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="md:col-span-4 flex flex-col gap-3">
                            <div className="bg-[#111] rounded-xl p-4 border border-[#2a2a2a] flex items-center justify-between hover:border-[#ff5757] hover:shadow-[0_0_15px_rgba(255,87,87,0.3)] hover:-translate-y-1 transition-all duration-300">
                                ...
                                <span className="text-2xl font-bold text-[#FF5757]">{completedWeekIds.length}</span>
                            </div>
                            <div className="bg-[#111] rounded-xl p-4 border border-[#2a2a2a] flex items-center justify-between hover:border-[#ff5757] hover:shadow-[0_0_15px_rgba(255,87,87,0.3)] hover:-translate-y-1 transition-all duration-300">
                                <span className="text-xs text-gray-400 uppercase tracking-widest">Remaining</span>
                                <span className="text-2xl font-bold text-gray-300">{remaining}</span>
                            </div>
                            <div className="bg-[#111] rounded-xl p-4 border border-[#2a2a2a] flex flex-col justify-center hover:border-[#ff5757] hover:shadow-[0_0_15px_rgba(255,87,87,0.3)] hover:-translate-y-1 transition-all duration-300">
                                <div className="flex justify-between items-center mb-2">
                                    ...
                                    <span className="text-lg font-bold text-[#ff5757]">{Math.round(progress)}%</span>
                                </div>
                                <div className="w-full h-1.5 bg-[#2a2a2a] rounded-full overflow-hidden">
                                    <div className="h-full bg-gradient-to-r from-[#ff5757] to-[#ff8888] fill-transition" style={{ width: `${progress}%` }} />
                                </div>
                            </div>
                        </div>
                    </section>
                        ...
                        <div className="space-y-2">
                            ...
                            {currentQuestions.length === 0 && (
                                <div className="text-center p-8 bg-[#111] rounded-xl border border-[#2a2a2a]">
                                    <p className="text-gray-400 text-sm">No questions found! Make sure you have completed questions.</p>
                                </div>
                            )}
                        </div>
                    </section>
...
export const getServerSideProps = async () => getPreFetchProps({ slug: "/revisions", appId: "dsayatra" });
