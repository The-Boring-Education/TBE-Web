...
function TopicsClient() {
    ...
    const nodes = useMemo(() => {
        ...
        // Preferred sequence from design
        const sortedKeys = Array.from(topicMap.keys()).sort((a, b) => {
            const order = ["ARRAY", "SLIDING_WINDOW", "RECURSION", "BINARY_SEARCH", "LINKED_LIST", "STACK", "STRING"];
        ...
        }).filter(node => node.total > 0 || node.name === "RECURSION" || node.isLocked);
    ...
    return (
        <div className="min-h-[calc(100vh-72px)] bg-[#0a0a0a] text-white relative font-sans overflow-x-hidden pt-4 pb-0 flex flex-col items-center">
            ...
            {/* Header Section */}
            <div className="w-full max-w-[1400px] flex justify-start px-8 absolute top-8 z-50">
                <button
                    onClick={() => router.push("/dashboard")}
                    className="flex items-center gap-2 px-4 py-2 rounded-lg border border-[#ff5757]/30 text-[#ff5757] hover:bg-[#ff5757]/10 transition-all font-bold text-sm group"
                >
            ...
            <motion.div
                className="relative z-10 flex flex-col items-center justify-center text-center px-6 mt-16 md:mt-4"
