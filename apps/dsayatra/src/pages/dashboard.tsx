...
function DsaClient() {
    ...
    const weeklyPerformance = useMemo(() => {
        ...
        if (weeklyLogs) {
            weeklyLogs.forEach((log: any) => {
                ...
                if (logDate >= joinDate) {
                    const daysOffset = Math.floor((logDate.getTime() - joinDate.getTime()) / (1000 * 60 * 60 * 24));
    ...
    const allRevisions = useMemo(() => {
        const questions: { id: string; title: string; completed: boolean; weekInfo: string }[] = [];
        Object.keys(weeklyAssignments).forEach((weekKey) => {
            ...
            assignments.forEach((id) => {
                const q = allQuestions.find((q: any) => String(q._id || q.id || q.name) === String(id));
                if (q) {
                    questions.push({
                        id: String(id),
    ...
    const completedRevisionsCount = allRevisions.filter((r) => r.completed).length;
                    ...
                    <div className="flex flex-col items-center">
                        ...
                        <div className="grid grid-cols-2 lg:grid-cols-4 gap-2.5 w-full max-w-2xl text-center mb-5 px-4">
                            {[
                                ...
                                {
                                    label: "Target",
                                    value: (profile as any)?.dsaYatra?.companies && (profile as any).dsaYatra.companies.length > 0
                ...
                <Card className="bg-[#1a1a1a] border-[#2a2a2a] p-3.5 rounded-[10px] hover:scale-[1.01] hover:shadow-[0_0_20px_rgba(255,87,87,0.15)] hover:border-[#ff5757]/40 transition-all duration-300">
                    <div className="mb-3.5">
                        ...
                        <p className="text-[10px] text-[#a0a0a0] mt-0.5">
                            {dueRevisionsCount} pending {dueRevisionsCount === 1 ? 'question' : 'questions'}
                        </p>
                    </div>
                        ...
                        {displayRevisions.length === 0 ? (
                            completedQuestions.length < 10 ? (
                                <div className="flex flex-col items-center justify-center py-6 text-center space-y-2 border border-dashed border-[#2a2a2a] rounded-lg bg-[#0f0f0f]/50">
                                    ...
                                    <p className="text-[14px] font-bold text-[#e0e0e0]">Revisions Locked!</p>
                                    <p className="text-[11px] text-[#a0a0a0] max-w-[220px]">
                                        Solve at least <span className="text-[#ff5757] font-bold">10 questions</span> to unlock your first set of revisions!
                                    </p>
                                    <span className="text-[#ff5757] font-semibold text-[10px] italic mt-2">
                                        You're at {completedQuestions.length}/10. Go crush some code! 🚀
                                    </span>
                                </div>
                            ) : (
                                <div className="flex flex-col items-center justify-center py-6 text-center space-y-2 border border-dashed border-[#2a2a2a] rounded-lg bg-[#0f0f0f]/50">
                    ...
                    <div className="mt-6 flex items-center justify-between">
                        <p className="text-[11px] text-[#a0a0a0]">{completedRevisionsCount} overall completed</p>
                        <Link href="/revisions" className="bg-[#ff6b6b] text-white inline-flex hover:bg-[#ff5252] text-[12px] font-[600] px-[18px] py-[8px] rounded-[6px] transition-all">
                            Go to Revisions
                        </Link>
                    </div>
                </Card>
