  ...
  try {
    ...
    if (!user || user.occupation !== 'DEVREL_ADVOCATE') {
      return res.status(apiStatusCodes.FORBIDDEN).json(
        sendAPIResponse({
          status: false,
          message: 'Access denied. Only DevRel Advocates can access this endpoint.',
        })
      );
    }
...
const handleGetApplications = async (req: NextApiRequest, res: NextApiResponse) => {
...
const handleUpdateApplication = async (req: NextApiRequest, res: NextApiResponse, userId: string) => {
  const { applicationId, status, notes, interviewDate, interviewLink } = req.body;
