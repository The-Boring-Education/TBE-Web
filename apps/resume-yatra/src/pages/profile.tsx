...
interface ResumeYatraProfilePageApiUser extends Omit<UserProfile, "name"> {
  name?: string;
  email?: string;
  image?: string;
}

const ResumeYatraProfilePage = () => {
  ...
  const resetFormFromSources = () => {
    ...
    setForm({
      ...
      experienceBand: rawRecord?.resumeYatra?.experienceBand ?? "student",
    });
  };
  ...
  const currentExperienceLabel =
    EXPERIENCE_OPTIONS.find((o) => o.value === form.experienceBand)?.label ||
    form.experienceBand;

  return (
    <Fragment>
      <Navbar variant="resume-yatra" profileRoute="/profile" />
      <main className="min-h-screen w-full overflow-x-hidden pt-[72px] bg-white sm:bg-slate-50 text-slate-700">
              ...
              {isEditing ? (
                <div className="space-y-6 pt-4 border-t border-slate-100 text-base">
                  ...
                  <div className="rounded-lg border border-slate-200 bg-slate-50/50 p-4 space-y-4">
                    ...
                    <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
                      ...
                      <div className="flex flex-col gap-1.5">
                        <label className="text-xs font-medium text-slate-500 flex items-center gap-1.5">
                          <FiGithub className="size-3.5 text-[#FF5757]" />{" "}
                          GitHub
                        </label>
                        <input
                          type="url"
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {EXPERIENCE_OPTIONS.map((opt) => {
                        const isSelected = form.experienceBand === opt.value;
                        return (
                          <button
                            key={opt.value}
                            type="button"
                            onClick={() =>
                              updateForm("experienceBand", opt.value)
                            }
                ...
                <div className="space-y-4 pt-4 border-t border-slate-100 text-base text-slate-700">
                  ...
                  <div className="flex items-center">
                    ...
                    <span className="text-slate-700 font-medium">
                      {currentExperienceLabel || "Not set"}
                    </span>
                  </div>
