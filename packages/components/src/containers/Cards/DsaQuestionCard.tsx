    ...
    return (
        ...
        >
            <div className="flex items-center space-x-2 flex-1 min-w-0 pr-2">
                ...
                <p className={`text-[13px] font-medium truncate ${isSelected ? 'text-white' : isCompleted ? 'text-green-50' : 'text-gray-300 group-hover:text-white'}`}>
                    {name}
                </p>
            </div>
            <span className={`text-[10px] font-medium px-2 py-0.5 flex-shrink-0 rounded-full border ${color}`}>
                {label}
            </span>
        </div>
    );
