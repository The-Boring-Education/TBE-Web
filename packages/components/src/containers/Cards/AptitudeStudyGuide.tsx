...
export const AptitudeStudyGuide: React.FC<AptitudeStudyGuideProps> = ({
    ...
    let rawSections = markdownContent.split(/\n\s*---\s*\n/).filter(s => s.trim());
    ...
    return (
        <div className="flex-1 flex flex-col min-w-0 w-full h-full max-h-full overflow-y-auto overflow-x-hidden scrollbar-thin-grey px-4 py-6 relative">
            <div className="max-w-2xl mx-auto w-full pb-8 interview-content-wrapper p-0 gap-4">
                ...
                <div className="question-header-card mb-2 p-5 rounded-2xl border border-gray-800/80 bg-[#111]" data-visible={isVisible} style={{ animationDelay: "0s" }}>
                    <div className="question-header-glow opacity-30" />
                    <div className="question-header-inner text-left">
                        <h1 className="question-main-title text-xl font-bold mb-1.5 tracking-tight">{topicName} Study Guide</h1>
                        <p className="question-subtitle text-xs text-gray-400">Review the key concepts, formulas, and strategies before taking the practice quiz.</p>
                    </div>
                </div>
                ...
                <div className="sections-container w-full flex flex-col gap-4">
                    {sections.map((sectionContent, index) => {
                        ...
                        let title = (titleMatch && titleMatch[1]) ? titleMatch[1].trim() : "Details";

                        // Map loose titles to exact config keys if needed
                        if (title.toLowerCase().includes("concepts")) title = "Core Concepts";
                        if (title.toLowerCase().includes("solving plan") || title.toLowerCase().includes("step-by-step")) title = "Step-by-Step Solving Plan";
                            ...
                            >
                                ...
                                <div className="relative flex flex-col p-5 md:p-6">
                                    <div className="flex items-center mb-4">
                                        <BoxedIcon path={config.iconPath} color={config.titleColor} />
                                        <h3 className="text-sm font-bold uppercase tracking-[0.15em] m-0" style={{ color: config.titleColor }}>
                                            {title}
                                        </h3>
                                        <div
                ...
                >
                    <FlexContainer className="justify-center items-center py-3 bg-[#111] rounded-xl border border-gray-800/60" fullWidth direction="col">
