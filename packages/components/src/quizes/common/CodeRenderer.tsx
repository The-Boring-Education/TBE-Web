    ...
    const normalizeMarkdown = (src: string): string => {
      ...
      if (src.includes("```")) {
        const fixed = src
          ...
          .replace(
            /```(javascript|js|typescript|ts|python|py|java|cpp|c|html|css|json|xml|sql|bash|sh)\s+/gi,
            "```$1\n",
          )
      ...
      if (backtickCount >= 10 && src.length > 120) {
        ...
        code = code.replace(
          /^\s*(javascript|js|typescript|ts|python|py|java|cpp|c|html|css|json|xml|sql|bash|sh)\s+/i,
          "",
        );
