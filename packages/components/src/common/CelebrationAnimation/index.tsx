...
const getRandomColor = (colorArray: string[]): string => {
  return colorArray[Math.floor(Math.random() * colorArray.length)] || "#000";
};
...
const CelebrationAnimation = ({
  ...
  // Strictly typed color palette
  const colors: Record<"points" | "levelup" | "achievement", string[]> = {
    points: ["#10b981", "#3b82f6", "#8b5cf6", "#f59e0b"], // Green, Blue, Purple, Amber
    levelup: ["#f59e0b", "#eab308", "#f97316", "#ef4444"], // Amber, Gold, Orange, Red
    achievement: ["#8b5cf6", "#a855f7", "#c084fc", "#e879f9"], // Purples and Pinks
  };

  // Number of particles based on intensity
  const particleCount: Record<"low" | "medium" | "high", number> = {
    low: 15,
    medium: 25,
    high: 40,
  };
  ...
  useEffect(() => {
    if (isActive) {
      ...
      const newParticles = Array.from({ length: count }, (_, i) => ({
        ...
        color: getRandomColor(colors[type]), // ✅ Always returns a string
