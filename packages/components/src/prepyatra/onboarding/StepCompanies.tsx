...
import {OnboardingData,  OnboardingCompanyType} from "@tbe/types";

type Props = {
    formData: OnboardingData
    toggleArrayField: (field: "targetCompanies", value: OnboardingCompanyType) => void
}
        ...
        <div className='grid grid-cols-1 sm:grid-cols-2 gap-4'>
            {COMPANY_TYPES.map((company) => (
                <button
                    ...
                    type='button'
                    onClick={() =>
                        toggleArrayField("targetCompanies", company.value as OnboardingCompanyType)
                    }
                    className={`p-4 border-2 rounded-xl text-left transition-all ${
                        formData.targetCompanies.includes(company.value as OnboardingCompanyType)
