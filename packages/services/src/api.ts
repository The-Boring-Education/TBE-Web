...
export const analyticsApi = {
  getPerformanceMetrics: async (
    ...
    try {
      return await apiClient.get<APIResponse<PerformanceMetrics>>(
        API_ENDPOINTS.QUIZ_PERFORMANCE(userId),
      );
    } catch (error) {
  ...
  getCategoryPerformance: async (
    ...
    try {
      return await apiClient.get<APIResponse<CategoryPerformance[]>>(
        API_ENDPOINTS.QUIZ_ANALYTICS(userId),
      );
    } catch (error) {
  ...
  getPerformanceHistory: async (
    ...
    try {
      return await apiClient.get<APIResponse<PerformanceHistory[]>>(
        API_ENDPOINTS.QUIZ_ANALYTICS(userId),
        {
...
export const leaderboardApi = {
  getLeaderboard: async (
    ...
    try {
      return await apiClient.get<APIResponse<LeaderboardData[]>>(
        API_ENDPOINTS.QUIZ_LEADERBOARD,
        {
  ...
  getUserRank: async (
    ...
    try {
      return await apiClient.get<APIResponse<{ rank: number }>>(
...
export const gamificationApi = {
  getuserGamificationPoints: async (
    userId: string,
  ): Promise<APIResponse<any>> => {
    try {
      const response = await apiClient.get<APIResponse<UserPoints>>(
        API_ENDPOINTS.GAMIFICATION_USER_POINTS,
        {
          params: { userId },
        },
      );
