...
export const buildEnvHealthResponse = (
    ...
    const results: EnvVarResult[] = checks.map((check) => {
        ...
        return {
            ...
            message: isPresent
                ? undefined
                : check.description || `${check.name} environment variable is not configured`,
        };
    });

    const missingRequired = results.filter((result) => !result.optional && result.status === 'missing');
    const missingOptional = results.filter((result) => result.optional && result.status === 'missing');

    const serviceStatus = getHealthStatusFromCounts(missingRequired.length, missingOptional.length);
