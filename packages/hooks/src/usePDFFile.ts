...
const PROGRAMMING_SKILLS = [
  ...
  'nextjs', 'next.js', 'nuxt', 'svelte', 'jquery', 'html', 'html5', 'css', 'css3',
  'sass', 'scss', 'less', 'tailwind', 'tailwindcss', 'bootstrap', 'material-ui', 'mui',
  // Backend
  'node', 'nodejs', 'node.js', 'express', 'expressjs', 'nestjs', 'fastapi', 'flask', 'django',
  'spring', 'spring boot', 'springboot', '.net', 'dotnet', 'asp.net', 'laravel', 'rails',
  // Databases
  'mongodb', 'mysql', 'postgresql', 'postgres', 'redis', 'cassandra', 'dynamodb', 
  'sqlite', 'oracle', 'mssql', 'sql server', 'firebase', 'firestore', 'elasticsearch',
  // Cloud & DevOps
  'aws', 'azure', 'gcp', 'google cloud', 'docker', 'kubernetes', 'k8s', 'jenkins',
  'gitlab', 'github actions', 'ci/cd', 'terraform', 'ansible', 'nginx', 'apache',
  // Mobile
  'react native', 'flutter', 'android', 'ios', 'swift', 'kotlin',
  // Tools & Others
  'git', 'github', 'gitlab', 'bitbucket', 'jira', 'webpack', 'vite', 'babel',
  'graphql', 'rest api', 'restful', 'grpc', 'websocket', 'microservices',
  'agile', 'scrum', 'tdd', 'jest', 'mocha', 'cypress', 'selenium',
  'redux', 'mobx', 'zustand', 'rxjs', 'socket.io'
];
...
const usePDFFile = () => {
  ...
  const extractTextFromPDF = async (file: File) => {
    console.log('🔍 extractTextFromPDF called with:', file.name, file.type, file.size);
    ...
    try {
      ...
      console.log('✅ pdfjs-dist imported successfully, version:', pdfjs.version);
