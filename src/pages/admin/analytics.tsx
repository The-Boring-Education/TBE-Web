...
const AdminAnalytics = () => {
  const { data: revenueData, loading: revenueLoading, fetchData: fetchRevenue } = useAdminData();
  const { data: engagementData, loading: engagementLoading, fetchData: fetchEngagement } = useAdminData();
  const { data: contentData, loading: contentLoading, fetchData: fetchContent } = useAdminData();
  const { data: gamificationData, loading: gamificationLoading, fetchData: fetchGamification } = useAdminData();
  const [dateRange, setDateRange] = useState('30d');

  useEffect(() => {
    const params = { period: dateRange };
    fetchRevenue('/api/v1/admin/analytics', { type: 'revenue', ...params });
    fetchEngagement('/api/v1/admin/analytics', { type: 'user-engagement', ...params });
    fetchContent('/api/v1/admin/analytics', { type: 'content-performance', ...params });
    fetchGamification('/api/v1/admin/analytics', { type: 'gamification', ...params });
  }, [fetchRevenue, fetchEngagement, fetchContent, fetchGamification, dateRange]);
  ...
  const stats = [
    ...
    },
    {
      ...
      value: `₹${((revenueData?.averageTransactionValue || 0) / 100).toLocaleString()}`,
      icon: <ArrowTrendingUpIcon className="h-6 w-6" />,
  ...
  const tabPanels = [revenueCharts, engagementCharts, contentCharts, gamificationCharts];
  const tabLabels = ['Revenue', 'User Engagement', 'Content Performance', 'Gamification'];
    ...
    <>
      ...
      <AdminLayout title="Analytics Dashboard">
        <div className="space-y-6">
          ...
          {/* Stats Cards */}
          <AdminStats stats={stats} loading={revenueLoading || engagementLoading} />
