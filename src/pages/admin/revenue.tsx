...
const AdminRevenue = () => {
  ...
  const { data: subscriptionData, loading: subscriptionLoading, fetchData: fetchSubscriptions } = useAdminData();
  const [dateRange, setDateRange] = useState('30d');

  useEffect(() => {
    const params = { period: dateRange };
    fetchRevenue('/api/v1/admin/analytics', { type: 'revenue', ...params });
    fetchSubscriptions('/api/v1/admin/analytics', { type: 'operational', ...params });
  ...
  const formatProductRevenue = (data: any) => {
    ...
    return data.productRevenue.map((item: any) => ({
      product: item._id === 'COURSE' ? 'Courses' : 
               item._id === 'PROJECT' ? 'Projects' : 
               item._id === 'INTERVIEW_SHEET' ? 'Interview Sheets' : 
               item._id,
      revenue: item.totalRevenue / 100,
      transactions: item.transactionCount,
    }));
            ...
            <div className="bg-white rounded-lg shadow p-6">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                                         {formatProductRevenue(revenueData).map((product: any, index: number) => (
                      <tr key={index} className="border-t">
                        ...
                        <td className="px-4 py-2 text-right">{product.transactions}</td>
                        <td className="px-4 py-2 text-right">
                          ₹{Math.round(product.revenue / product.transactions || 0).toLocaleString()}
                        </td>
                      </tr>
                    ))}
                  </tbody>
