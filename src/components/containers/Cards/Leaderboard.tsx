...
const Leaderboard = () => {
  ...
  const ProfileImage = ({ src, name, isTopThree }: { src?: string; name: string; isTopThree: boolean }) => {
            ...
            return (
              ...
              >
                <FlexContainer className="flex items-center gap-3">
                  {renderRank(idx + 1)}
                  <ProfileImage src={entry.userId.image} name={name} isTopThree={isTopThree} />
                  <div className="flex flex-col">
                    <Text level="p" className={`${nameStyle} leading-tight`}>
                      {name}
                    </Text>
                    {isUser && <span className="text-xs text-blue-600 font-medium">You</span>}
                  </div>
                </FlexContainer>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className={`text-sm ${isTopThree ? "text-gray-600 font-medium" : "text-gray-500"}`}>pts</span>
                </div>
              </motion.div>
            )
          })}
    ...
    return (
      ...
      >
        <div className="flex items-center justify-center gap-2 text-gray-700">
          ...
          <span className="font-bold text-blue-700 bg-blue-100 px-2 py-1 rounded-full text-sm">#{userRank}</span>
          <span>with</span>
          <span className="font-bold text-blue-700 bg-blue-100 px-2 py-1 rounded-full text-sm">{entry.points} pts</span>
        </div>
      </motion.div>
    )
  }
  ...
  return (
    <Section className="w-full max-w-3xl mx-auto mt-8 bg-white border border-gray-200 shadow-xl rounded-2xl overflow-hidden">
      ...
      <div className="px-6 py-4 border-b border-gray-100">
        <Text level="h1" className="text-2xl font-bold text-gray-900 text-center">
          Leaderboard
        </Text>
      </div>
      {/* Tab Navigation */}
