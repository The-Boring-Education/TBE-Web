...
const AdminTable = ({
  ...
  const toggleAllRows = () => {
    if (selectedRows.size === data.length) {
      ...
      setSelectedRows(new Set(data.map((item, index) => item._id || index.toString())));
    }
  };
  ...
  const handleSort = (key: string) => {
    ...
    const newOrder = sorting.sortBy === key && sorting.order === 'asc' ? 'desc' : 'asc';
        ...
        <div className="p-4 border-b border-gray-200">
          <div className="flex justify-between items-center">
            ...
            <div className="flex items-center space-x-2">
              {actions?.onRefresh && (
                                               <Button variant="OUTLINE" text="Refresh" onClick={actions.onRefresh} />
              )}
              {actions?.onExport && (
                                               <Button variant="OUTLINE" text="Export" onClick={actions.onExport} />
              )}
            </div>
          </div>
        </div>
              ...
              <tr key={row._id || index} className="hover:bg-gray-50">
                <td className="px-6 py-4 whitespace-nowrap">
                  <input
                    ...
                    onChange={() => toggleRowSelection(row._id || index.toString())}
                    className="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                  />
                </td>
                {columns.map((column) => (
